...
async function saveUserProfile(userId) {
  ...
  const { error } = await withTimeout(
    supabaseClient.from("profiles").update({ full_name: name, role, lab }).eq("id", userId),
    12000,
...
async function saveSupabaseRequest(record, isExisting) {
  ...
  if (isExisting) {
    ({ error } = await supabaseClient.from("giu_requests").update(payload).eq("id", record.id));
  } else {
    payload.created_by = currentUser.id;
    payload.updated_by = currentUser.id;
    ({ error } = await supabaseClient.from("giu_requests").insert(payload));
  }
...
function withTimeout(promise, timeoutMs, message) {
  return Promise.race([
    promise,
    new Promise((resolve) => {
      window.setTimeout(() => resolve({ error: { message } }), timeoutMs);
    }),
  ]);
}
